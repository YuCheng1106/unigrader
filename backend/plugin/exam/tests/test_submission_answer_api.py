...
from backend.common.response import ResponseModel
...
class TestSubmissionAnswerApi:
    ...
    async def test_create_submission_answer(self, submission_answer_client: AsyncClient):
        ...
        response = await submission_answer_client.post('/v1/sys/submission_answer', json=json_data)
    ...
    async def test_get_submission_answer_detail(self, submission_answer_client: AsyncClient):
        """测试获取提交答案详情"""
        response = await submission_answer_client.get('/v1/sys/submission_answer/1')
        assert response.status_code == 200
        response_data: ResponseModel = response.json()
        assert response_data['code'] == 200
    ...
    async def test_get_submission_answers_by_submission_id(self, submission_answer_client: AsyncClient):
        """测试通过提交ID获取所有答案"""
        response = await submission_answer_client.get('/v1/sys/submission_answer/submission/1')
        assert response.status_code == 200
        response_data: ResponseModel = response.json()
        assert response_data['code'] == 200
    ...
    async def test_get_submission_answers_by_user_id(self, submission_answer_client: AsyncClient):
        """测试通过用户ID获取所有答案"""
        response = await submission_answer_client.get('/v1/sys/submission_answer/user/1')
        assert response.status_code == 200
        response_data: ResponseModel = response.json()
        assert response_data['code'] == 200
        assert isinstance(response_data['data'], list)
    ...
    async def test_get_submission_answers_by_exam_id(self, submission_answer_client: AsyncClient):
        """测试通过考试ID获取所有答案"""
        response = await submission_answer_client.get('/v1/sys/submission_answer/exam/1')
        assert response.status_code == 200
        response_data: ResponseModel = response.json()
        assert response_data['code'] == 200
        assert isinstance(response_data['data'], list)
    ...
    async def test_get_submission_answer_by_user_and_question(self, submission_answer_client: AsyncClient):
        """测试通过用户ID和题目ID获取答案"""
        response = await submission_answer_client.get('/v1/sys/submission_answer/user/1/question/1')
        assert response.status_code == 200
        response_data: ResponseModel = response.json()
        assert response_data['code'] == 200
    ...
    async def test_get_submission_answers_by_grader(self, submission_answer_client: AsyncClient):
        """测试通过批改者ID获取所有批改的答案"""
        response = await submission_answer_client.get('/v1/sys/submission_answer/grader/1')
        assert response.status_code == 200
        response_data: ResponseModel = response.json()
        assert response_data['code'] == 200
        assert isinstance(response_data['data'], list)
    ...
    async def test_update_submission_answer(self, submission_answer_client: AsyncClient):
        ...
        response = await submission_answer_client.put('/v1/sys/submission_answer/1', json=json_data)
        assert response.status_code == 200
        response_data: ResponseModel = response.json()
        assert response_data['code'] == 200
    ...
    async def test_delete_submission_answer(self, submission_answer_client: AsyncClient):
        """测试删除提交答案"""
        response = await submission_answer_client.delete('/v1/sys/submission_answer/1')
        assert response.status_code == 200
        response_data: ResponseModel = response.json()
        assert response_data['code'] == 200
    ...
    async def test_delete_submission_answers_batch(self, submission_answer_client: AsyncClient):
        """测试批量删除提交答案"""
        json_data = {'pks': [2, 3]}
        response = await submission_answer_client.delete('/v1/sys/submission_answer', json=json_data)
        assert response.status_code == 200
        response_data: ResponseModel = response.json()
        assert response_data['code'] == 200
    ...
    async def test_get_submission_answer_not_found(self, submission_answer_client: AsyncClient):
        """测试获取不存在的提交答案"""
        response = await submission_answer_client.get('/v1/sys/submission_answer/999')
    ...
    async def test_update_submission_answer_not_found(self, submission_answer_client: AsyncClient):
        ...
        response = await submission_answer_client.put('/v1/sys/submission_answer/999', json=json_data)
        assert response.status_code == 404
        response_data: ResponseModel = response.json()
        assert response_data['code'] == 404
        assert '提交答案不存在' in response_data['msg']
    ...
    async def test_delete_submission_answer_not_found(self, submission_answer_client: AsyncClient):
        """测试删除不存在的提交答案"""
        response = await submission_answer_client.delete('/v1/sys/submission_answer/999')
        assert response.status_code == 404
        response_data: ResponseModel = response.json()
        assert response_data['code'] == 404
        assert '提交答案不存在' in response_data['msg']